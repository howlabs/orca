...
describe('registerAppMenu', () => {
  ...
  it.runIf(!isMac)('puts Settings and Exit under File on Windows/Linux', () => {
    ...
    expect(helpLabels.indexOf('Getting started')).toBeLessThan(helpLabels.indexOf('Explore Orca'))
  ...
  it.runIf(!isMac)('routes Getting started through its callback on Windows/Linux', () => {
    const options = buildMenuOptions()
    registerAppMenu(options)

    const gettingStartedItem = getSubmenu(getTemplate(), 'Help').find(
      (entry) => entry.label === 'Getting started'
    )
    expect(gettingStartedItem?.accelerator).toBeUndefined()

    const targetWindow = {} as Electron.BaseWindow
    gettingStartedItem?.click?.({} as never, targetWindow, {} as Electron.KeyboardEvent)
