...
describe('Store', () => {
  ...
  it('returns default settings when no data file exists', async () => {
    ...
    expect(settings.sourceControlGroupOrder).toBeUndefined()
    ...
    expect(settings.showMobileButton).toBe(false)
    expect(settings.showMobileButtonDefaultedOffForAllUsers).toBe(true)
  ...
  it('marks upgrade-cohort completion and persisted dismissal gate on first load', async () => {
    writeDataFile({
      schemaVersion: 1,
      ui: {}
    })
  ...
  it('writes upgrade-cohort completion gate flags to disk', async () => {
  ...
  it('leaves dismissal gate false while first-run flow is open', async () => {
    writeDataFile({
      onboarding: {
  ...
  it('forces dismissal gate true when flow is closed and stored dismissal was false', async () => {
    writeDataFile({
      onboarding: {
        flowVersion: ONBOARDING_FLOW_VERSION,
        closedAt: 123,
  ...
  it('coerces malformed completed flow and sets dismissal gate on load', async () => {
    writeDataFile({
      onboarding: {
        flowVersion: ONBOARDING_FLOW_VERSION,
        closedAt: 'yesterday',
  ...
  it('keeps dismissal gate true when dismissed flow has null closedAt', async () => {
    writeDataFile({
      onboarding: {
        flowVersion: ONBOARDING_FLOW_VERSION,
        closedAt: null,
  ...
  it('merges loaded data with defaults for missing fields', async () => {
    ...
    expect(store.getSettings().showAutomationsButtonDefaultedOffForAllUsers).toBe(true)
    expect(store.getSettings().showMobileButton).toBe(false)
    expect(store.getSettings().showMobileButtonDefaultedOffForAllUsers).toBe(true)
  ...
  it('updateSettings persists sourceControlGroupOrder as a user setting', async () => {
    const store = await createStore()
    expect(store.getSettings().sourceControlGroupOrder).toBeUndefined()
  ...
  it('defaults showAutomationsButton and showMobileButton off for legacy profiles once', async () => {
    writeDataFile({
      schemaVersion: 1,
      repos: [],
      worktreeMeta: {},
  ...
  it('preserves showAutomationsButton and showMobileButton after the default-off migration has run', async () => {
    writeDataFile({
      schemaVersion: 1,
      repos: [],
      worktreeMeta: {},
      settings: {
        ...
        showMobileButton: true,
        showMobileButtonDefaultedOffForAllUsers: true
      },
      ui: {},
      githubCache: { pr: {}, issue: {} },
      workspaceSession: {}
    ...
    expect(store.getSettings().showMobileButton).toBe(true)
  })
