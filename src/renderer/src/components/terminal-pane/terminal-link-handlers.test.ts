...
describe('createFilePathLinkProvider range bounds', () => {
  ...
  it('opens a wrapped continuation-row html path from a direct modifier-click fallback', async () => {
    setPlatform('Macintosh')
    const rows = [
      makeBufferLine('open design-'),
      makeBufferLine('preview.html', { isWrapped: true })
    ]
    ...
    expect(createBrowserTabMock).toHaveBeenCalledWith(
      'wt-1',
      'file:///tmp/design-preview.html',
      expect.objectContaining({ title: 'design-preview.html', activate: true })
    )
    expect(openFilePathMock).not.toHaveBeenCalled()
  })
