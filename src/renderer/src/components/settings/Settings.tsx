...
} from '@/lib/getting-started-settings-search'
...
function getSettingsNavGroupDefinitionsForSearch(
  ...
  return sections.flatMap((section) => {
    if (seenGroupIds.has(section.group)) {
      return []
    }
...
function Settings(): React.JSX.Element {
  ...
  // Why: Terminal owns visual controls, but the Ghostty import flow
  ...
  const pendingSectionId = pendingNavSectionRef.current
  const visibleSectionIds = useMemo(() => {
    const next = new Set(visibleNavSections.map((section) => section.id))
    const repoSectionIds = new Set(repos.map((repo) => `repo-${repo.id}`))
    if (activeSectionId && repoSectionIds.has(activeSectionId)) {
      next.add(activeSectionId)
    }
    if (pendingSectionId && repoSectionIds.has(pendingSectionId)) {
      next.add(pendingSectionId)
    }
    return next
  }, [activeSectionId, pendingSectionId, repos, visibleNavSections])
  ...
  const repoSectionById = useMemo(() => {
    const next = new Map<string, SettingsNavSection>()
    for (const repo of repos) {
      ...
      next.set(`repo-${repo.id}`, {
        ...
        searchEntries: getRepositoryPaneSearchEntries(repo, { isLocalWindowsProject }),
        group: 'workflows'
      })
    }
    return next
  }, [isWindowsTerminalHost, repos])
  const getSectionSearchEntries = (sectionId: string) => {
    const repoSection = repoSectionById.get(sectionId)
    if (repoSection) {
      return getSettingsSectionSearchEntries(repoSection)
    }
    ...
    async (
      ...
      if (
        tryOpenGettingStartedFromSettingsSearch({
          ...
          openModal,
          clearSearch: () => setSettingsSearchQuery('')
        })
      ) {
        return
      }
  ...
  const generalNavGroups: SettingsNavGroup[] = generalNavGroupDefinitions
    .map((group) => ({
      ...
      sections: visibleNavSections.filter((section) =>
        shouldIncludeSectionInSettingsSidebarGroup({
          sectionId: section.id,
          groupId: group.id,
          sectionGroupId: section.group,
          searchQuery: settingsSearchQuery
        })
      )
    }))
              ...
              <ActiveSettingsSectionProvider value={activeSectionId}>
                ...
                <SettingsSection
                  ...
                  description={translate(
                    ...
                    'Shells, themes, typography, renderer, sessions, and behavior.'
                  )}
                ...
                <SettingsSection
                  ...
                  description={translate(
                    ...
                    'Theme, zoom, font, language, and app icon.'
                  )}
